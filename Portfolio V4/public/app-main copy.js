// /Portfolio V4/public/app-main.js
/**
...
} from './event-handlers/_navigation.js';
...
async function initialize() {
  ...
  try {
    const storedSettings =
      JSON.parse(localStorage.getItem('stockTrackerSettings')) || {};
    ...
    const newSettings = {
      ...
      numberOfDateTabs: 1, // Ensure defaults exist
      ...storedSettings,
    };
    updateState({ settings: newSettings }); // Use updateState
    console.log('[App Main] Loaded settings:', state.settings);
    applyAppearanceSettings();
    // Explicitly set the title after settings are applied
    const appTitleElement = document.getElementById('app-title');
    if (appTitleElement) {
      appTitleElement.textContent = 'Dev Strategy Lab';
    }
  } catch (e) {
  ...
  if (!mainContent || !modalContainer) {
    console.error(
      '[App Main] Fatal: Main content or modal container not found.'
    );
    return;
  }
  ...
  const cacheBust = '?v=3.0.18'; // Incremented version
  // --- END FIX ---
  ...
  try {
    ...
    const [
      ...
      await Promise.all([
        // Page fetches
        fetch('./templates/_alerts.html' + cacheBust).then((res) =>
          res.ok
            ? res.text()
            : Promise.reject(`_alerts.html: ${res.statusText}`)
        ),
        fetch('./templates/_dailyReport.html' + cacheBust).then((res) =>
          res.ok
            ? res.text()
            : Promise.reject(`_dailyReport.html: ${res.statusText}`)
        ),
        fetch('./templates/_imports.html' + cacheBust).then((res) =>
          res.ok
            ? res.text()
            : Promise.reject(`_imports.html: ${res.statusText}`)
        ),
        fetch('./templates/_ledger.html' + cacheBust).then((res) =>
          res.ok
            ? res.text()
            : Promise.reject(`_ledger.html: ${res.statusText}`)
        ),
        fetch('./templates/_orders.html' + cacheBust).then((res) =>
          res.ok
            ? res.text()
        ...
        fetch('./templates/_research.html' + cacheBust).then((res) =>
          res.ok
            ? res.text()
            : Promise.reject(`_research.html: ${res.statusText}`)
        ),
        fetch('./templates/_dashboard.html' + cacheBust).then((res) =>
          res.ok
            ? res.text()
            : Promise.reject(`_dashboard.html: ${res.statusText}`)
        ),
        // Modal fetches
        fetch('./templates/_modal_advice.html' + cacheBust).then((res) =>
          res.ok
            ? res.text()
            : Promise.reject(`_modal_advice.html: ${res.statusText}`)
        ),
        fetch('./templates/_modal_settings.html' + cacheBust).then((res) =>
          res.ok
            ? res.text()
            : Promise.reject(`_modal_settings.html: ${res.statusText}`)
        ),
        fetch('./templates/_modal_edit_transaction.html' + cacheBust).then(
          (res) =>
            res.ok
              ? res.text()
              : Promise.reject(
                  `_modal_edit_transaction.html: ${res.statusText}`
                )
        ),
        fetch('./templates/_modal_confirm.html' + cacheBust).then((res) =>
          res.ok
            ? res.text()
            : Promise.reject(`_modal_confirm.html: ${res.statusText}`)
        ),
        fetch('./templates/_modal_sell_from_position.html' + cacheBust).then(
          (res) =>
            res.ok
              ? res.text()
              : Promise.reject(
                  `_modal_sell_from_position.html: ${res.statusText}`
                )
        ),
        fetch('./templates/_modal_confirm_fill.html' + cacheBust).then(
          (res) =>
            res.ok
              ? res.text()
              : Promise.reject(`_modal_confirm_fill.html: ${res.statusText}`)
        ),
        fetch('./templates/_modal_sales_history.html' + cacheBust).then(
          (res) =>
            res.ok
              ? res.text()
              : Promise.reject(`_modal_sales_history.html: ${res.statusText}`)
        ),
        fetch('./templates/_modal_selective_sell.html' + cacheBust).then(
          (res) =>
            res.ok
              ? res.text()
              : Promise.reject(`_modal_selective_sell.html: ${res.statusText}`)
        ),
        fetch('./templates/_modal_manage_position.html' + cacheBust).then(
          (res) =>
            res.ok
              ? res.text()
              : Promise.reject(`_modal_manage_position.html: ${res.statusText}`)
        ),
        fetch('./templates/_modal_source_details.html' + cacheBust).then(
          (res) =>
            res.ok
              ? res.text()
              : Promise.reject(`_modal_source_details.html: ${res.statusText}`)
        ),
        fetch('./templates/_modal_image_zoom.html' + cacheBust).then((res) =>
          res.ok
            ? res.text()
            : Promise.reject(`_modal_image_zoom.html: ${res.statusText}`)
        ),
        fetch('./templates/_modal_add_trade_idea.html' + cacheBust).then(
          (res) =>
            res.ok
              ? res.text()
              : Promise.reject(`_modal_add_trade_idea.html: ${res.statusText}`)
        ),
        fetch('./templates/_modal_add_technique.html' + cacheBust).then(
          (res) =>
            res.ok
              ? res.text()
              : Promise.reject(`_modal_add_technique.html: ${res.statusText}`)
        ),
        fetch('./templates/_modal_add_paper_trade.html' + cacheBust).then(
          (res) =>
            res.ok
              ? res.text()
    ...
    console.error(
      '[App Main] Failed to load or inject one or more templates:',
      error
    );
    showToast(
      `Error loading UI templates (${error}). Please refresh.`,
      'error',
      30000
    );
  ...
  try {
    ...
    console.log('[App Main] initializeAllEventHandlers() called.'); // Added log
  } catch (error) {
    console.error(
      '[App Main] Error during initializeAllEventHandlers():',
      error
    );
  ...
  console.log(
    '[App Main] Fetching initial global data (holders, exchanges)...'
  );
  try {
    await Promise.all([
    ...
    showToast(
      'Error loading account data. Some features may be limited.',
      'error'
    );
  }
  ...
  try {
    ...
    if (
      typeof defaultHolderIdSetting === 'string' &&
      !isNaN(parseInt(defaultHolderIdSetting))
    ) {
    ...
    // @ts-ignore
    const holderExists = state.allAccountHolders.some(
      (h) => h.id === defaultHolderId
    );
    if (!holderExists && defaultHolderId !== 1) {
      console.warn(
        `[App Main] Default holder ID ${defaultHolderId} from settings not found, defaulting to 1 (Primary).`
      );
      defaultHolderId = 1;
    }
    ...
    const globalFilter = /** @type {HTMLSelectElement} */ (
      document.getElementById('global-account-holder-filter')
    );
    if (globalFilter) {
      // Check if the option *value* exists (comparing strings)
      const optionExists = Array.from(globalFilter.options).some(
        (opt) => opt.value === String(state.selectedAccountHolderId)
      );
      if (optionExists) {
        globalFilter.value = String(state.selectedAccountHolderId);
        console.log(
          `[App Main] Set global filter to ID: ${globalFilter.value}`
        );
      } else {
        // This case should ideally not happen if validation above works, but as a fallback:
        console.warn(
          `[App Main] Option for holder ID ${state.selectedAccountHolderId} not found in dropdown, setting to 1.`
        );
      ...
      console.warn(
        '[App Main] Global account holder filter dropdown not found.'
      );
    }
  } catch (e) {
  ...
  try {
    console.log(
      `[App Main] Fetching initial advice sources for default holder: ${state.selectedAccountHolderId}`
    );
  ...
  setTimeout(async () => {
    try {
      ...
      console.log(
        `[App Main] Successfully switched to default view: ${defaultViewType}`
      );
    } catch (error) {
      console.error(
        `[App Main] Error switching to default view (${defaultViewType}):`,
        error
      );
      showToast(
        `Failed to load default view (${defaultViewType}). Please try selecting a tab manually.`,
        'error'
      );
      ...
      if (defaultViewType !== 'dashboard') {
        ...
        try {
          ...
          console.error(
            '[App Main] Fallback to dashboard view also failed:',
            fallbackError
          );
