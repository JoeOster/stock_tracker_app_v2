...
export function showConfirmationModal(title, body, onConfirm) {
  ...
  const newConfirmBtn = /** @type {HTMLButtonElement} */ (
...
export function populatePricesFromCache(dataMap, priceCache) {
  const totalValueSummarySpan = document.querySelector(
    '#total-value-summary span'
  );
  const dashboardTotalValueSpan = document.querySelector(
    '#dashboard-total-value span'
  );
  ...
  dataMap.forEach((lot, key) => {
    ...
    if (
      ...
      if (priceCell)
        priceCell.innerHTML = `<span class="negative">${priceToUse === null || priceToUse === undefined ? '--' : priceToUse === 'invalid' ? 'Invalid' : 'Error'}</span>`;
      ...
      unrealizedPercent =
        costOfRemaining !== 0 ? (unrealizedPL / costOfRemaining) * 100 : 0;
  ...
  if (totalValueSummarySpan) {
    totalValueSummarySpan.innerHTML = `<strong>${formatAccounting(totalPortfolioValue)}</strong>`;
  }
  if (dashboardTotalValueSpan) {
    dashboardTotalValueSpan.innerHTML = `<strong>${formatAccounting(totalPortfolioValue)}</strong>`;
  }

  const dailyReportTotalPlCell = document.getElementById('unrealized-pl-total');
  const dashboardTotalPlCell = document.getElementById(
...
export function sortTableByColumn(th, tbody) {
  ...
  const allHeaders =
    th.closest('thead')?.querySelectorAll('th[data-sort]') ?? [];
