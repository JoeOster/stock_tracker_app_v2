...
async function handleCreateJournalEntry(db, body) {
  ...
  if (
    ...
    throw new Error(
      'Missing required fields for journal entry (holder, ticker, date, price, qty, exchange, direction).'
    );
  }
  // --- END MODIFICATION ---
  ...
  const result = await db.run(
    `INSERT INTO journal_entries (
            ...
            exchange, direction, advice_source_details, entry_reason, image_path, page_number, chapter
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, // --- ADDED ? ---
    [
      account_holder_id,
...
async function handleUpdateJournalEntry(db, id, body) {
  ...
  if (status === 'EXECUTED' && existingEntry.status !== 'EXECUTED') {
    throw new Error(
      "Cannot set status to 'EXECUTED' via the update endpoint. Use the /execute endpoint."
    );
  }
