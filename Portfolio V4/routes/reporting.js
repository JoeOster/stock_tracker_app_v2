...
module.exports = (db, log) => {
  ...
  router.get('/positions/:date', async (req, res) => {
    try {
      ...
      const dailyTransactionsQuery = `
                ...
                LEFT JOIN transactions AS parent_tx ON daily_tx.parent_buy_id = parent_tx.id
      ...
      dailyTransactions.forEach((tx) => {
        if (tx.transaction_type === 'SELL' && tx.parent_buy_price) {
          ...
          // @ts-ignore
          tx.realizedPL = tx.price * tx.quantity;
        }
      });
  ...
  router.get('/realized_pl/summary', async (req, res) => {
    try {
      ...
      const query = `
                SELECT exchange, SUM(total_pl) as total_pl
                FROM (
                    ...
                    WHERE s.transaction_type = 'SELL' ${holderFilter.replace('s.', '')} GROUP BY s.exchange

                    UNION ALL

                    SELECT exchange, SUM(price * quantity) as total_pl
                    FROM transactions
                    WHERE transaction_type = 'DIVIDEND' ${holderFilter} GROUP BY exchange
                )
                GROUP BY exchange;
            `;
    ...
    try {
      ...
      const query = `
        ...
        FROM (
            ...
            AND s.transaction_date <= ?
            ${holderFilter.replace('account_holder_id', 's.account_holder_id')}
            GROUP BY s.exchange
      ...
      if (accountHolderId && accountHolderId !== 'all') {
        queryParams.push(accountHolderId);
      }
      ...
      const byExchangeRows = await db.all(query, queryParams);
      const byExchange = byExchangeRows.map((row) => ({
        exchange: row.exchange,
        total_pl: row.total_pl,
      }));
