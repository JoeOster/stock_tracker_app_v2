...
async function handleSingleLotSell(db, log, txData, createdAt) {
  ...
  const parentBuy = await db.get(
    "SELECT * FROM transactions WHERE id = ? AND account_holder_id = ? AND transaction_type = 'BUY'",
    [parent_buy_id, account_holder_id]
  );
  if (!parentBuy) {
    throw new Error(
      'Parent buy transaction not found for this account holder.'
    );
  }
  ...
  if (parentBuy.quantity_remaining < numQuantity - 0.00001) {
    throw new Error(
      `Sell quantity (${internalFormatQuantity(numQuantity)}) exceeds remaining quantity (${internalFormatQuantity(parentBuy.quantity_remaining)}) in the selected lot.`
    );
  }
  ...
  await db.run(sellQuery, [
    ...
    'MANUAL',
    createdAt,
    parentBuy.advice_source_id,
    parentBuy.linked_journal_id,
  ]);
...
async function handleSelectiveSell(db, log, txData, createdAt) {
  const {
    ...
    account_holder_id,
    lots,
    quantity, // 'quantity' is the expected total
  } = txData;
  ...
  for (const lotInfo of lots) {
    ...
    const parentBuy = await db.get(
      "SELECT * FROM transactions WHERE id = ? AND account_holder_id = ? AND transaction_type = 'BUY'",
      [lotInfo.parent_buy_id, account_holder_id]
    );
    if (!parentBuy) {
      throw new Error(
        `Parent buy transaction (ID: ${lotInfo.parent_buy_id}) not found for this account holder.`
      );
    }
    if (new Date(transaction_date) < new Date(parentBuy.transaction_date)) {
      throw new Error(
        `Sell date cannot be before the buy date of lot ID ${lotInfo.parent_buy_id}.`
      );
    }
    if (parentBuy.quantity_remaining < lotQty - 0.00001) {
      throw new Error(
        `Sell quantity (${internalFormatQuantity(lotQty)}) exceeds remaining quantity (${internalFormatQuantity(parentBuy.quantity_remaining)}) in lot ID ${lotInfo.parent_buy_id}.`
      );
    }
  ...
  if (
    !isNaN(expectedTotalQuantity) &&
    Math.abs(totalSellQuantityFromLots - expectedTotalQuantity) > 0.00001
  ) {
    throw new Error(
      'Total quantity specified does not match the sum of quantities entered for individual lots.'
    );
  }
...
async function handleSellTransaction(
  ...
  // --- Case 3: Invalid SELL payload ---
    ...
    throw new Error(
      'Invalid SELL transaction payload. Must provide either a single parent_buy_id or a valid lots array.'
    );
  }
  ...
  if (
    ...
    typeof captureEodPrices === 'function'
  ) {
