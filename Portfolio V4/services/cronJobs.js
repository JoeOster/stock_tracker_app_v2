...
async function backupDatabase(db, log) {
  ...
  try {
    ...
    console.error(
      `[Cron Backup] Failed to backup database to ${backupDir}: ${error.message}`
    );
  }
}
 ...
 * @param {function(string): void} log - Logging function.
 */
async function captureEodPrices(db, dateToProcess, log) {
  // <-- ADDED log
  try {
    ...
    if (tickers.length === 0) {
      log(
        `[Cron EOD] No new EOD prices needed for held tickers on ${dateToProcess}.`
      ); // <-- FIX
      return;
    }

    log(
      `[Cron EOD] Fetching EOD prices for held tickers on ${dateToProcess}: ${tickers.join(', ')}`
    ); // <-- FIX
    ...
    for (const ticker of tickers) {
      ...
      if (priceInfo && typeof priceInfo.price === 'number') {
        ...
        log(
          `[Cron EOD] Saved EOD price for ${ticker} on ${dateToProcess}: ${priceInfo.price}`
        ); // <-- FIX
      } else {
        log(
          `[Cron EOD] Could not retrieve EOD price for ${ticker} on ${dateToProcess}.`
        ); // <-- FIX (changed from console.warn)
      }
    }
  } catch (error) {
    console.error(
      `[Cron EOD] Error capturing EOD prices for ${dateToProcess}: ${error.message}`
    );
  }
}
// --- END MODIFICATION ---
 ...
 * @param {function(string): void} log - Logging function.
 */
async function runWatcher(db, log) {
  // Renamed from runOrderWatcher
  try {
    log('[Cron Watcher] Starting watcher cycle...');
    // --- Fetch Active Items ---
    const activeBuyOrders = await db.all(
      "SELECT * FROM pending_orders WHERE status = 'ACTIVE' AND order_type = 'BUY_LIMIT'"
    );
    // Fetch journal entries with targets OR stops
    const openJournalEntries = await db.all(
      "SELECT * FROM journal_entries WHERE status = 'OPEN' AND (target_price IS NOT NULL OR stop_loss_price IS NOT NULL)"
    );
    ...
    if (tickersToWatch.length === 0) {
      log(
        '[Cron Watcher] No active orders or journal entries with targets/stops to watch.'
      );
      return;
    }
    ...
    for (const order of activeBuyOrders) {
      ...
      if (currentPrice <= order.limit_price) {
        ...
        if (!existingNotification) {
          ...
          log(
            `[Cron Watcher] Notification created for BUY_LIMIT order ID ${order.id} (${order.ticker})`
          );
        }
      }
    }
    ...
    for (const entry of openJournalEntries) {
      ...
      if (targetMet) {
        ...
        if (!existingNotification) {
          ...
          log(
            `[Cron Watcher] Notification created for Journal Entry ID ${entry.id} (${entry.ticker} hit ${targetMet})`
          );
        }
      }
    }
    ...
    console.error(
      `[Cron Error] runWatcher failed: ${error.message}\n${error.stack}`
    );
  }
}
...
function setupCronJobs(db, log) {
  ...
  if (process.env.NODE_ENV !== 'test') {
    ...
    log(
      'Cron jobs scheduled: EOD Price Capture, Order/Journal Watcher, Nightly Backups.'
    );
  } else {
