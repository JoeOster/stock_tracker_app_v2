...
module.exports = (db, log) => {
    ...
    router.delete('/holders/:id', async (req, res) => {
        try {
            ...
            const inUseLinks = await db.get('SELECT 1 FROM account_source_links WHERE account_holder_id = ? LIMIT 1', id);
            if (inUseLinks) {
                log(`[DELETE] Deleting links for holder ${id} before deleting holder.`);
                await db.run('DELETE FROM account_source_links WHERE account_holder_id = ?', id);
            }
            // --- END FIX ---
    ...
    router.get('/holders/:id/sources', async (req, res) => {
        ...
        try {
            ...
            log(`[ERROR] Failed to fetch source subscription list for holder ${id}: ${error.message}`);
            res.status(500).json({ message: 'Error fetching source subscription list.' });
        }
    });
    ...
    router.put('/holders/:id/sources', async (req, res) => {
        ...
        if (!Array.isArray(sourceIds)) {
            return res.status(400).json({ message: 'An array of sourceIds is required.' });
        }
        ...
        try {
            ...
            if (sourceIds.length > 0) {
                const stmt = await db.prepare('INSERT INTO account_source_links (account_holder_id, advice_source_id) VALUES (?, ?)');
            ...
            res.json({ message: `Source subscriptions updated for account holder ${id}.` });
        } catch (error) {
            await db.exec('ROLLBACK');
            log(`[ERROR] Failed to update source subscriptions for holder ${id}: ${error.message}`);
