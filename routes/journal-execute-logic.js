...
async function handleExecuteJournalEntry(db, log, id, body) {
  ...
  if (!execution_date || !execution_price || !account_holder_id) {
    throw new Error(
      'Missing required execution data: date, price, and account holder ID.'
    );
  }
  ...
  const entry = await db.get(
    'SELECT * FROM journal_entries WHERE id = ? AND account_holder_id = ?',
  ...
  if (entry.status !== 'OPEN') {
    throw new Error(
      `Cannot execute a journal entry with status: ${entry.status}.`
    );
  }
  ...
  try {
    ...
    if (source && source.name) {
      ...
      if (source.name.toLowerCase().includes('robinhood'))
        exchange = 'Robinhood';
      if (source.name.toLowerCase().includes('etrade')) exchange = 'E-Trade';
    }
  } catch (e) {
    log(
      `[EXECUTE_JOURNAL] Could not find advice source to determine exchange: ${e.message}`
    );
  }
  ...
  log(
    `[EXECUTE_JOURNAL] Created new transaction ID: ${newTransactionId} from journal entry ID: ${id}`
  );
