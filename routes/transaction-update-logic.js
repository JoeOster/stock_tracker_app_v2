...
async function handleUpdateTransaction(db, id, txData) {
  ...
  if (originalTx.transaction_type === 'BUY') {
    const qtyMatch =
      Math.abs(originalTx.quantity_remaining - originalTx.original_quantity) <
      0.00001;
    const qtyDifference = numQuantity - originalTx.original_quantity;

    if (qtyMatch || qtyDifference > 0) {
      const newQuantityRemaining =
        originalTx.quantity_remaining + qtyDifference;
      await db.run(
        'UPDATE transactions SET original_quantity = ?, quantity_remaining = ? WHERE id = ?',
        [numQuantity, newQuantityRemaining, id]
      );
    } else {
      await db.run(
        'UPDATE transactions SET original_quantity = ? WHERE id = ?',
        [numQuantity, id]
      );
    }
  }
