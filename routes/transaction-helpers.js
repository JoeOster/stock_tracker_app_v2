  ...
  transactions.forEach((tx) => {
    ...
    const priceKey =
      tx.price !== null && tx.price !== undefined
        ? tx.price.toFixed(5)
        : 'null_price'; // Use fixed decimal places for key
...
function findConflict(parsedRow, existingTransactions) {
  ...
  for (const tx of existingTransactions) {
    ...
    const quantityMatch =
      Math.abs(parsedRow.quantity - tx.quantity) < QTY_TOLERANCE_ABSOLUTE;
    ...
    if (
      ...
      priceMatch =
        (Math.abs(parsedRow.price - tx.price) / tx.price) * 100 <
        PRICE_TOLERANCE_PERCENT;
    } else if (parsedRow.price === tx.price) {
      // Handle cases where both might be null or zero
      priceMatch = true;
    }
...
// --- *** ADDED THIS FUNCTION *** ---
/**
 * Archives a watchlist item if it's linked to the parent BUY lot or advice source.
 * This is now called by both BUY and SELL.
 * @param {import('sqlite').Database} db - The database connection object.
 * @param {function(string): void} log - The logging function.
 * @param {object} tx - The BUY transaction object.
 * @param {string|number} accountHolderId - The account holder ID.
 * @param {string} ticker - The ticker symbol.
 */
async function archiveWatchlistItem(db, log, tx, accountHolderId, ticker) {
  // An item can be linked via advice_source_id OR a specific journal_entry_id
  const adviceSourceId = tx.advice_source_id;
  const journalEntryId = tx.linked_journal_id;

  if (adviceSourceId || journalEntryId) {
    log(
      `[TRANSACTION] Archiving watchlist item for Ticker: ${ticker}, Source: ${adviceSourceId}, Journal: ${journalEntryId}`
    );

    let query = `UPDATE watchlist SET status = 'CLOSED' 
                 WHERE account_holder_id = ? AND ticker = ? AND status = 'OPEN' AND (`;
    const params = [accountHolderId, ticker.toUpperCase()];

    const conditions = [];
    if (adviceSourceId) {
      conditions.push('advice_source_id = ?');
      params.push(adviceSourceId);
    }
    if (journalEntryId) {
      conditions.push('journal_entry_id = ?');
