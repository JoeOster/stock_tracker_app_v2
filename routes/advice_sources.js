...
module.exports = (db, log) => {
    ...
    router.post('/', async (req, res) => {
        ...
        try {
            ...
            const newSource = await db.get('SELECT * FROM advice_sources WHERE name = ? AND type = ?', [name, type]);
    ...
    router.delete('/:id', async (req, res) => {
        ...
        if (!holderId || holderId === 'all') {
            return res.status(400).json({ message: 'A specific account holder ID is required to remove a source.' });
        }
        ...
        try {
            ...
            if (result.changes === 0) {
                return res.status(404).json({ message: 'Source link not found for this user.' });
            }
            
            // 2. (Optional) Garbage collect the source if no one else is using it.
            const links = await db.get('SELECT COUNT(*) as count FROM account_source_links WHERE advice_source_id = ?', id);
            if (links.count === 0) {
                ...
                if (journalCount.count === 0 && txCount.count === 0) {
                    log(`[DELETE] Source ${id} is no longer linked by any user or item. Deleting globally.`);
                    await db.run('DELETE FROM advice_sources WHERE id = ?', id);
                    // Note: Cascades will delete from source_notes.
                } else {
                    log(`[DELETE] Source ${id} unlinked, but kept globally as it's still referenced by ${journalCount.count} journal entries or ${txCount.count} transactions.`);
                }
            }
            // --- END FIX ---
            ...
            log(`[ERROR] Failed to delete/unlink advice source ${id}: ${e.message}\n${e.stack}`);
