...
// FIX: Make sure importSessions is passed into the module
module.exports = (db, log, captureEodPrices, importSessions) => {
    ...
    router.post('/import', async (req, res) => {
        ...
        if (!sessionId || !Array.isArray(resolutions)) {
            return res.status(400).json({ message: 'Invalid import payload. Session ID or resolutions missing.' });
        }
        ...
        // Process user's choices for conflicting transactions
                ...
                for (const tx of toCreate) {
                    ...
                    if (tx.type === 'BUY') {
                        ...
                        if (openLots.length === 0) {
                             await db.exec('ROLLBACK'); // Abort the transaction before sending error
                             return res.status(400).json({ message: `Import failed: Cannot import a SELL for ${tx.ticker} because no open BUY lots were found. Please import your BUYs first.` });
                        }
    ...
    router.post('/', async (req, res) => {
        try {
            ...
            if (transaction_type === 'SELL' && !parent_buy_id) {
                return res.status(400).json({ message: 'A parent buy lot must be selected for a SELL transaction.' });
            }
