...
describe('Account Holders API', () => {
  ...
  it('should fetch all account holders', async () => {
    await db.run(
      "INSERT INTO account_holders (name) VALUES ('Holder A'), ('Holder B')"
    );
  ...
  it('should update an account holder successfully', async () => {
    ...
    const updatedHolder = await db.get(
  ...
  it('should delete an unused account holder successfully', async () => {
    ...
    const deletedHolder = await db.get(
      'SELECT * FROM account_holders WHERE id = ?',
      holderId
    );
  ...
  it('should prevent deleting the primary account holder (ID 1)', async () => {
    ...
    expect(res.body.message).toBe(
      'Cannot delete the default Primary account holder.'
    );
  });
  ...
  it('should prevent deleting an account holder that has transactions', async () => {
    ...
    const holderId = holderRes.lastID;
    await db.run(
      'INSERT INTO transactions (ticker, exchange, transaction_type, quantity, price, transaction_date, account_holder_id) VALUES (?, ?, ?, ?, ?, ?, ?)',
      ['TEST', 'TestEx', 'BUY', 100, 10, '2025-10-01', holderId]
    );
...
describe('Exchanges API', () => {
  ...
  it('should prevent deleting an exchange that is in use', async () => {
    await db.run(
      'INSERT INTO transactions (ticker, exchange, transaction_type, quantity, price, transaction_date, account_holder_id) VALUES (?, ?, ?, ?, ?, ?, ?)',
      ['TEST', 'Fidelity', 'BUY', 10, 10, '2025-10-01', 1]
    );

    const fidelity = await db.get(
      "SELECT id FROM exchanges WHERE name = 'Fidelity'"
    );

    const res = await request(app).delete(
      `/api/accounts/exchanges/${fidelity.id}`
    );
    expect(res.statusCode).toEqual(400);
    expect(res.body.message).toBe(
