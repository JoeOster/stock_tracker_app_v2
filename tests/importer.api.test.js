...
beforeEach(async () => {
  ...
  await db.run(
    "INSERT OR IGNORE INTO account_holders (id, name) VALUES (2, 'Test Holder')"
  );
});
...
describe('Importer API', () => {
  it('should correctly process a CSV, identify conflicts, and commit changes', async () => {
    ...
    await db.run(
      ...
      ['2025-10-10', 'CSV-CONFLICT', 'Fidelity', 'BUY', 5, 100.0, 2, 'MANUAL']
    );
    ...
    expect(uploadRes.body.reconciliationData.newTransactions[0].ticker).toBe(
      'CSV-NEW'
    );
    expect(uploadRes.body.reconciliationData.conflicts[0].csvData.ticker).toBe(
      'CSV-CONFLICT'
    );
    ...
    const allTxs = await db.all(
      'SELECT * FROM transactions ORDER BY ticker ASC'
    );
  ...
  it('should successfully parse a live Fidelity CSV file', async () => {
    ...
    expect(uploadRes.statusCode).toEqual(200);
    expect(
      uploadRes.body.reconciliationData.newTransactions.length
    ).toBeGreaterThan(0);
  });
