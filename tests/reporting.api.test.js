...
describe('Reporting Endpoints', () => {
  it('should correctly calculate the total realized P&L', async () => {
    const buyRes = await db.run(
      'INSERT INTO transactions (ticker, exchange, transaction_type, quantity, price, transaction_date, original_quantity, quantity_remaining, account_holder_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)',
      ['PL-TEST', 'TestEx', 'BUY', 10, 100, '2025-10-08', 10, 10, 2]
    );
    const buyTransactionId = buyRes.lastID;
    await db.run(
      'INSERT INTO transactions (ticker, exchange, transaction_type, quantity, price, transaction_date, parent_buy_id, account_holder_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
      ['PL-TEST', 'TestEx', 'SELL', 8, 120, '2025-10-09', buyTransactionId, 2]
    );

    const res = await request(app).get(
      '/api/reporting/realized_pl/summary?holder=2'
    );
  ...
  it('should correctly calculate realized P&L within a date range', async () => {
    const buyRes = await db.run(
      'INSERT INTO transactions (ticker, exchange, transaction_type, quantity, price, transaction_date, original_quantity, quantity_remaining, account_holder_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)',
      ['RNG-TEST', 'TestEx', 'BUY', 10, 100, '2025-09-01', 10, 0, 2]
    );
    await db.run(
      'INSERT INTO transactions (ticker, exchange, transaction_type, quantity, price, transaction_date, parent_buy_id, account_holder_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
      ['RNG-TEST', 'TestEx', 'SELL', 10, 120, '2025-10-05', buyRes.lastID, 2]
    );
  ...
  it('should return 0 for P&L when there are no sales in the date range', async () => {
    ...
    await db.run(
      'INSERT INTO transactions (ticker, exchange, transaction_type, quantity, price, transaction_date, parent_buy_id, account_holder_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
      ['RNG-TEST', 'TestEx', 'SELL', 10, 120, '2025-11-05', buyRes.lastID, 2]
    );
  ...
  it('should correctly calculate the weighted average cost basis', async () => {
    await db.run(
      'INSERT INTO transactions (ticker, exchange, transaction_type, quantity, price, transaction_date, original_quantity, quantity_remaining, account_holder_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)',
      ['CALC-TEST', 'TestEx', 'BUY', 100, 10, '2025-10-01', 100, 100, 2]
    );
    await db.run(
      'INSERT INTO transactions (ticker, exchange, transaction_type, quantity, price, transaction_date, original_quantity, quantity_remaining, account_holder_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)',
      ['CALC-TEST', 'TestEx', 'BUY', 50, 12, '2025-10-02', 50, 50, 2]
    );
    mockedFetch.mockResolvedValue({
      ok: true,
      json: () => Promise.resolve({ c: 15 }),
    });

    const res = await request(app).get(
      '/api/reporting/portfolio/overview?holder=2'
    );
  ...
  it('should return empty arrays when fetching positions for a date with no transactions', async () => {
    const res = await request(app).get(
      '/api/reporting/positions/2025-01-01?holder=2'
    );
    expect(res.statusCode).toEqual(200);
