...
describe('Transaction API Endpoints', () => {
  ...
  it('should create a new SELL transaction and update the parent BUY lot', async () => {
    const buyRes = await db.run(
      ...
      ['SELL-TEST', 'TestEx', 'BUY', 100, 50, '2025-10-01', 100, 100, 2]
    );
    const buyId = buyRes.lastID;

    const sellRes = await request(app).post('/api/transactions').send({
      ticker: 'SELL-TEST',
      exchange: 'TestEx',
    ...
    const parentBuyLot = await db.get(
  ...
  it('should fail to create a SELL transaction with a date before the buy date', async () => {
    const buyRes = await db.run(
      'INSERT INTO transactions (ticker, exchange, transaction_type, quantity, price, transaction_date, original_quantity, quantity_remaining, account_holder_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)',
      ['SELL-TEST', 'TestEx', 'BUY', 100, 50, '2025-10-05', 100, 100, 2]
    );
    const buyId = buyRes.lastID;
    ...
    const sellRes = await request(app).post('/api/transactions').send({
      ...
      quantity: 50,
      price: 60,
      transaction_date: '2025-10-04', // Date is before buy date
      account_holder_id: 2,
      parent_buy_id: buyId,
    });

    expect(sellRes.statusCode).toEqual(400);
    expect(sellRes.body.message).toBe(
      'Sell date cannot be before the buy date.'
    );
  });
  ...
  it('should successfully delete a SELL and restore the quantity to the parent BUY', async () => {
    ...
    const buyRes = await db.run(
      'INSERT INTO transactions (ticker, exchange, transaction_type, quantity, price, transaction_date, original_quantity, quantity_remaining, account_holder_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)',
      ['DEL-TEST', 'TestEx', 'BUY', 100, 50, '2025-10-01', 100, 60, 2]
    );
    const buyId = buyRes.lastID;
    const sellRes = await db.run(
      ...
      ['DEL-TEST', 'TestEx', 'SELL', 40, 60, '2025-10-02', buyId, 2]
    );
