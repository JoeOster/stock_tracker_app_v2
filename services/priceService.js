...
// --- Read both API keys ---
const API_KEYS = [process.env.FINNHUB_API_KEY, process.env.FINNHUB_API_KEY_2].filter(Boolean);
let currentKeyIndex = 0;

// --- Adjust API Calls per Minute ---
const TOTAL_API_CALLS_PER_MINUTE = (API_KEYS.length > 1 && parseInt(process.env.API_CALLS_PER_MINUTE, 10))
...
async function fetchSinglePrice(ticker, apiKey) {
    try {
        ...
        console.log(`[${getTimestamp()}] [Price Service] Submitting request for ${ticker} using key ending in ${apiKey.slice(-4)}`);
        ...
        if (apiRes.ok) {
            ...
            console.warn(`[${getTimestamp()}] [Price Service Warning] Ticker '${ticker}' returned invalid price data: ${JSON.stringify(data)}`);
            return 'invalid';
        }
...
async function getPrices(tickers, priority = 5) {
    if (API_KEYS.length === 0) {
        console.error(`[${getTimestamp()}] [Price Service Error] No API keys configured.`);
    ...
    if (tickersToFetch.length > 0) {
        ...
        const fetchPromises = tickersToFetch.map(ticker => {
            ...
            return limiter.schedule({ priority }, () => fetchSinglePrice(ticker, keyToUse));
        });
