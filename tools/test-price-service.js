...
const { getPrices, disconnect } = require(path.resolve(__dirname, '..', 'services/priceService.js'));

const tickersToTest = ['INTC', 'GOOGL'];

async function testApi() {
    console.log(`[TEST] Attempting to fetch prices for: ${tickersToTest.join(', ')}`);
    console.log(`[TEST] Using API Key 1 ending in: ${process.env.FINNHUB_API_KEY ? '...' + process.env.FINNHUB_API_KEY.slice(-4) : 'Not Set'}`);
    if (process.env.FINNHUB_API_KEY_2) {
         console.log(`[TEST] Using API Key 2 ending in: ${'...' + process.env.FINNHUB_API_KEY_2.slice(-4)}`);
    }
     console.log(`[TEST] API Call Limit set in .env: ${process.env.API_CALLS_PER_MINUTE || 'Not Set (Defaults based on key count)'}`);
