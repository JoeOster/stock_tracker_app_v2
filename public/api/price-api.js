...
export async function updatePricesForView(viewDate, tickersToUpdate) {
  ...
  try {
    ...
    for (const ticker in pricesData) {
      ...
      const previousPrice =
        currentCached && typeof currentCached.price === 'number'
          ? currentCached.price
          : null;

      const newPriceValue = pricesData[ticker];
      const newPrice =
        typeof newPriceValue === 'number' && newPriceValue > 0
          ? newPriceValue
          : 'invalid';
      ...
      const newPriceData = {
        price: newPrice,
        previousPrice:
          newPrice === 'invalid' ? currentCached?.previousPrice : previousPrice,
        timestamp: now,
      };
    ...
    tickersToUpdate.forEach((ticker) => {
      ...
      // @ts-ignore
      state.priceCache.set(ticker, {
        price: 'error',
        previousPrice: existingPrevious ?? null,
        timestamp: Date.now(),
      });
    });
  }
...
export async function updateAllPrices() {
  ...
  if (
    ...
    state.dashboardOpenLots.length > 0
  ) {
    tickersToUpdate = [
      ...new Set(state.dashboardOpenLots.map((lot) => lot.ticker)),
    ];
    ...
    const journalTickers = state.journalEntries?.openEntries
      ? state.journalEntries.openEntries.map((entry) => entry.ticker)
      : [];
    const recommendedTickers = state.researchWatchlistItems
      ? state.researchWatchlistItems.map((item) => item.ticker)
      : [];
  ...
  if (tickersToUpdate.length > 0 && dateForUpdate) {
    ...
    if (state.currentView.type === 'dashboard') {
      const { renderDashboardPage } = await import(
        '../ui/renderers/_dashboard_render.js'
      );
