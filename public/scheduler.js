...
async function runScheduler(appState) {
  const refreshPricesBtn = /** @type {HTMLButtonElement | null} */ (
    document.getElementById('refresh-prices-btn')
  );
  ...
  const needsLiveUpdates =
    (appState.currentView.type === 'date' && appState.activityMap.size > 0) ||
  ...
  let currentIntervalMinutes = isMarketHours
    ? marketHoursIntervalMinutes
    : afterHoursIntervalMinutes;
  ...
  if (isMarketHours) {
    ...
    let secondsRemaining = Math.max(
      0,
      Math.round((nextApiCallTimestamp - Date.now()) / 1000)
    );
  ...
  if (Date.now() >= nextApiCallTimestamp) {
    console.log(
      `[${new Date().toISOString()}] Scheduler triggered update for on-screen tickers...`
    );
    ...
    if (appState.currentView.type === 'date' && appState.activityMap.size > 0) {
      tickersToUpdate = [
        ...new Set(
          Array.from(appState.activityMap.values()).map((lot) => lot.ticker)
        ),
      ];
      dateForUpdate = appState.currentView.value; // Keep specific date for daily view
    } else if (appState.currentView.type === 'research') {
      const journalTickers = appState.journalEntries?.openEntries
        ? appState.journalEntries.openEntries.map((entry) => entry.ticker)
        : [];
      const recommendedTickers = appState.researchWatchlistItems
        ? appState.researchWatchlistItems.map((item) => item.ticker)
        : [];
      tickersToUpdate = [
        ...new Set([...journalTickers, ...recommendedTickers]),
      ];
    ...
    if (tickersToUpdate.length > 0 && dateForUpdate) {
      try {
        ...
        if (appState.currentView.type === 'date') {
          ...
          const { loadResearchPage } = await import(
            './event-handlers/_research.js'
          );
