...
import { saveSubscriptions, populateSubscriptionPanel } from '../event-handlers/_modal_manage_subscriptions.js';
...
export async function saveExchangeChange(li) {
    ...
    try {
        ...
        const currentSaveBtn = document.querySelector(`.save-exchange-btn[data-id="${id}"]`);
        if (currentSaveBtn) {
...
export function initializeExchangeManagementHandlers() {
    ...
    if (exchangeList) {
        exchangeList.addEventListener('click', async (e) => {
            ...
            const li = /** @type {HTMLLIElement | null} */ (target.closest('li[data-id]'));
            if (!li) return;
            const id = li.dataset.id;
            if (!id) return;
            const nameSpan = /** @type {HTMLElement | null} */ (li.querySelector('.exchange-name'));
            ...
            if (target === editBtn) {
                ...
                try {
                    ...
                    console.log("Inline save failed, error was already shown by saveExchangeChange.");
                }
            } 
             ...
             if (subBtn) {
                 ...
                 if (holderId && holderName) {
                     ...
                     if (userMgmtPanel && subTabsContainer && subTabButton) {
                         setActiveTab( // <-- FIX: This function is now imported
                             subTabsContainer,
                ...
                try {
                 ...
                 showConfirmationModal(`Delete Account Holder "${holderName}"?`, 'This cannot be undone and will fail if the holder has transactions.', async () => {
                    try {
                        ...
                        await fetchAndPopulateAccountHolders();
                        renderAccountHolderManagementList(); // <-- FIX: Call local function
...
async function commitPendingEdits() {
    const savePromises = [];
    const pendingHolderSaves = document.querySelectorAll('#account-holder-list .save-holder-btn');
    pendingHolderSaves.forEach(btn => {
        if ((/** @type {HTMLElement} */(btn)).style.display !== 'none') {
            const li = (/** @type {HTMLLIElement} */(btn)).closest('li');
            if (li) {
                console.log(`[SaveSettings] Found pending edit for holder: ${li.dataset.id}`);
                savePromises.push(saveHolderChange(li));
            }
        }
    });

    const pendingExchangeSaves = document.querySelectorAll('#exchange-list .save-exchange-btn');
    pendingExchangeSaves.forEach(btn => {
        if ((/** @type {HTMLElement} */(btn)).style.display !== 'none') {
            const li = (/** @type {HTMLLIElement} */(btn)).closest('li');
            if (li) {
                console.log(`[SaveSettings] Found pending edit for exchange: ${li.dataset.id}`);
    ...
    if (savePromises.length > 0) {
        console.log(`[SaveSettings] Awaiting ${savePromises.length} pending inline saves...`);
...
export async function saveSettings() {
    try {
        ...
        // @ts-ignore
        console.error(`[SaveSettings] Failed to save pending edits or subscriptions: ${error.message}`);
