...
function renderDashboardUI(
  ...
  const message = isEmpty
    ...
    : isFilteredEmpty
      ...
      : 'Loading...'; // Adjusted message

  // Render Aggregated Cards
  if (aggregatedLots.length === 0 && (isEmpty || isFilteredEmpty)) {
    // Show message only if truly empty or filtered empty
  ...
  if (individualLotsForTable.length === 0 && (isEmpty || isFilteredEmpty)) {
    // Show message only if truly empty or filtered empty
    ...
    tableBody.innerHTML = individualLotsForTable
