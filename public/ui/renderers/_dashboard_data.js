            ...
            case 'proximity-asc': {
                const getProximityPercent = (lot) => {
                    ...
                    if (currentPriceNum !== null && currentPriceNum > 0) {
                         if (lot.limit_price_up && currentPriceNum < lot.limit_price_up) { // Check if below limit_up
                            proxPercent = Math.min(proxPercent, ((lot.limit_price_up - currentPriceNum) / currentPriceNum) * 100);
                        }
                        if (lot.limit_price_down && currentPriceNum > lot.limit_price_down) { // Check if above limit_down
                           proxPercent = Math.min(proxPercent, ((currentPriceNum - lot.limit_price_down) / currentPriceNum) * 100);
                        }
                    }
