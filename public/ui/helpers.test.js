...
// Import functions
import { populatePricesFromCache, showToast, showConfirmationModal, sortTableByColumn } from './helpers.js';
// Removed unused import: import { getTradingDays } from './datetime.js';

// --- Mocks ---
jest.mock('./formatters.js', () => ({ // Correct path
    formatAccounting: jest.fn((num) => (num === null || num === undefined || isNaN(num) ? '--' : (num === 0 ? '$0.00' : (num < 0 ? `($${Math.abs(num).toFixed(2)})` : `$${num.toFixed(2)}`)) )),
    formatQuantity: jest.fn((num) => (num === null || num === undefined || isNaN(num) ? '' : String(num))),
}));
jest.mock('./datetime.js', () => ({ // Correct path
...
jest.mock('../event-handlers/_journal_settings.js', () => ({ // Go up one level
    fetchAndStoreAdviceSources: jest.fn().mockResolvedValue(undefined)
}));
// --- FIX: Correct the path to journal-settings.js ---
jest.mock('./journal-settings.js', () => ({ // Correct path relative to helpers.test.js
...
const { formatAccounting, formatQuantity } = require('./formatters');
...
describe('UI Helper Functions', () => {
    ...
    describe('populatePricesFromCache', () => {
        ...
        test('should correctly calculate and render positive P/L', () => {
            ...
            const plCell = document.querySelector('tr[data-key="lot-1"] .unrealized-pl-combined');
         ...
         test('should correctly calculate and render negative P/L', () => {
            const activityMap = new Map([['lot-2', { ticker: 'MSFT', quantity_remaining: 5, cost_basis: 200 }]]);
            const priceCache = new Map([['MSFT', { price: 180, timestamp: Date.now() }]]);
            populatePricesFromCache(activityMap, priceCache);
            const plCell = document.querySelector('tr[data-key="lot-2"] .unrealized-pl-combined');
        ...
        test('should handle invalid price data in cache', () => {
            const activityMap = new Map([['lot-3', { ticker: 'BAD', quantity_remaining: 5, cost_basis: 50 }]]);
            const priceCache = new Map([['BAD', { price: 'invalid', timestamp: Date.now() }]]);
            populatePricesFromCache(activityMap, priceCache);
            const priceCell = document.querySelector('tr[data-key="lot-3"] .current-price');
            const plCell = document.querySelector('tr[data-key="lot-3"] .unrealized-pl-combined');
            const totalValueSpan = document.querySelector('#total-value-summary span');
