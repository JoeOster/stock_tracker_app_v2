...
document.addEventListener('DOMContentLoaded', () => {
    ...
    transactionForm.addEventListener('submit', async (e) => {
        e.preventDefault();
        const formData = { transaction_date: document.getElementById('transaction-date').value, ticker: document.getElementById('ticker').value.toUpperCase(), exchange: document.getElementById('exchange').value, transaction_type: document.getElementById('transaction-type').value, quantity: parseFloat(document.getElementById('quantity').value), price: parseFloat(document.getElementById('price').value) };
    ...
    const allOtherFunctions = () => {
        ...
        ledgerTableBody.addEventListener('click', async (e) => { const id = e.target.dataset.id; if (!id) return; if (e.target.classList.contains('delete-btn')) { if (e.target.dataset.type === 'snapshot') { if (confirm('Are you sure you want to delete this snapshot?')) { await fetch(`/api/snapshots/${id}`, { method: 'DELETE' }); renderOverviewPage(); } } else { if (confirm('Are you sure you want to delete this transaction?')) { await fetch(`/api/transactions/${id}`, { method: 'DELETE' }); handleTabClick('overview', null); } } } if (e.target.classList.contains('modify-btn')) { const tx = allTransactions.find(t => t.id == id); if (tx) { document.getElementById('edit-id').value = tx.id; document.getElementById('edit-date').value = tx.transaction_date; document.getElementById('edit-ticker').value = tx.ticker; const exchangeSelect = document.getElementById('edit-exchange'); exchangeSelect.innerHTML = document.getElementById('exchange').innerHTML; exchangeSelect.value = tx.exchange; document.getElementById('edit-type').value = tx.transaction_type; document.getElementById('edit-quantity').value = tx.quantity; document.getElementById('edit-price').value = tx.price; editModal.classList.add('visible'); } } });
        editForm.addEventListener('submit', async (e) => { e.preventDefault(); const id = document.getElementById('edit-id').value; const updatedTx = { transaction_date: document.getElementById('edit-date').value, ticker: document.getElementById('edit-ticker').value.toUpperCase(), exchange: document.getElementById('edit-exchange').value, transaction_type: document.getElementById('edit-type').value, quantity: parseFloat(document.getElementById('edit-quantity').value), price: parseFloat(document.getElementById('edit-price').value) }; await fetch(`/api/transactions/${id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(updatedTx) }); editModal.classList.remove('visible'); handleTabClick('overview', null); });
