...
function applyJournalFilters(readOnly = false) {
  ...
  const closedFilter = closedFilterInput
    ? closedFilterInput.value.toUpperCase()
    : '';
  const statusFilter = closedStatusSelect ? closedStatusSelect.value : '';

  const { openEntries, closedEntries } = state.journalEntries || {
    openEntries: [],
    closedEntries: [],
  };

  // Filter Open Entries
  const filteredOpen = openEntries.filter(
    (entry) => !openFilter || entry.ticker.toUpperCase().includes(openFilter)
  );

  // Filter Closed Entries
  const filteredClosed = closedEntries.filter((entry) => {
    const tickerMatch =
      !closedFilter || entry.ticker.toUpperCase().includes(closedFilter);
  ...
  renderJournalPage(
    { openEntries: filteredOpen, closedEntries: filteredClosed },
    readOnly
  );
}
...
export function initializeJournalFilterHandlers() {
  ...
  const closedFilterInput = document.getElementById(
    'journal-closed-filter-ticker'
  );
  const closedStatusSelect = document.getElementById(
    'journal-closed-filter-status'
  );
  // --- END MODIFICATION ---
  ...
  if (openFilterInput) {
    openFilterInput.addEventListener('input', () =>
      applyJournalFilters(isReadOnly)
    );
  }
  if (closedFilterInput) {
    closedFilterInput.addEventListener('input', () =>
      applyJournalFilters(isReadOnly)
    );
  }
  if (closedStatusSelect) {
    closedStatusSelect.addEventListener('change', () =>
      applyJournalFilters(isReadOnly)
    );
  }
}
