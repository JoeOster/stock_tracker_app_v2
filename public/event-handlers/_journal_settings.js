...
export async function fetchAndStoreAdviceSources() {
  ...
  try {
    ...
    const sources = await fetchAdviceSources(
      state.selectedAccountHolderId,
      false
    );
...
function getSourceDetailsFromForm(type, formPrefix) {
  ...
  switch (type) {
    case 'Person':
      // @ts-ignore
      details.contact_email = /** @type {HTMLInputElement} */ (
        document.getElementById(`${formPrefix}-contact-email`)
      ).value;
      // @ts-ignore
      details.contact_phone = /** @type {HTMLInputElement} */ (
        document.getElementById(`${formPrefix}-contact-phone`)
      ).value;
      // @ts-ignore
      details.contact_app_type = /** @type {HTMLSelectElement} */ (
        document.getElementById(`${formPrefix}-contact-app-type`)
      ).value;
      // @ts-ignore
      details.contact_app_handle = /** @type {HTMLInputElement} */ (
        document.getElementById(`${formPrefix}-contact-app-handle`)
      ).value;
      break;
    case 'Group':
      // @ts-ignore
      details.contact_person = /** @type {HTMLInputElement} */ (
        document.getElementById(`${formPrefix}-group-person`)
      ).value;
      // @ts-ignore
      details.contact_email = /** @type {HTMLInputElement} */ (
        document.getElementById(`${formPrefix}-group-email`)
      ).value;
      // @ts-ignore
      details.contact_phone = /** @type {HTMLInputElement} */ (
        document.getElementById(`${formPrefix}-group-phone`)
      ).value;
      // @ts-ignore
      details.contact_app_type = /** @type {HTMLSelectElement} */ (
        document.getElementById(`${formPrefix}-group-app-type`)
      ).value;
      // @ts-ignore
      details.contact_app_handle = /** @type {HTMLInputElement} */ (
        document.getElementById(`${formPrefix}-group-app-handle`)
      ).value;
      break;
    case 'Book':
      // @ts-ignore
      details.author = /** @type {HTMLInputElement} */ (
        document.getElementById(`${formPrefix}-book-author`)
      ).value;
      // @ts-ignore
      details.isbn = /** @type {HTMLInputElement} */ (
        document.getElementById(`${formPrefix}-book-isbn`)
      ).value;

      websites = /** @type {HTMLTextAreaElement} */ (
        document.getElementById(`${formPrefix}-book-websites`)
      ).value;
      pdfs = /** @type {HTMLTextAreaElement} */ (
        document.getElementById(`${formPrefix}-book-pdfs`)
      ).value;
      // @ts-ignore
    ...
    case 'Website':
      websites = /** @type {HTMLTextAreaElement} */ (
        document.getElementById(`${formPrefix}-website-websites`)
      ).value;
      pdfs = /** @type {HTMLTextAreaElement} */ (
        document.getElementById(`${formPrefix}-website-pdfs`)
      ).value;
      // @ts-ignore
      details.websites = websites
        ? websites
  ...
  switch (type) {
    case 'Person':
      ...
      /** @type {HTMLInputElement} */ (
        document.getElementById('edit-source-contact-email')
      ).value = d.contact_email || '';
      // @ts-ignore
      /** @type {HTMLInputElement} */ (
        document.getElementById('edit-source-contact-phone')
      ).value = d.contact_phone || '';
      // @ts-ignore
      /** @type {HTMLSelectElement} */ (
        document.getElementById('edit-source-contact-app-type')
      ).value = d.contact_app_type || '';
      // @ts-ignore
      /** @type {HTMLInputElement} */ (
        document.getElementById('edit-source-contact-app-handle')
      ).value = d.contact_app_handle || '';
      break;
    case 'Group':
      // @ts-ignore
      /** @type {HTMLInputElement} */ (
        document.getElementById('edit-source-group-person')
      ).value = d.contact_person || '';
      // @ts-ignore
      /** @type {HTMLInputElement} */ (
        document.getElementById('edit-source-group-email')
      ).value = d.contact_email || '';
      // @ts-ignore
      /** @type {HTMLInputElement} */ (
        document.getElementById('edit-source-group-phone')
      ).value = d.contact_phone || '';
      // @ts-ignore
      /** @type {HTMLSelectElement} */ (
        document.getElementById('edit-source-group-app-type')
      ).value = d.contact_app_type || '';
      // @ts-ignore
      /** @type {HTMLInputElement} */ (
        document.getElementById('edit-source-group-app-handle')
      ).value = d.contact_app_handle || '';
      break;
    case 'Book':
      // @ts-ignore
      /** @type {HTMLInputElement} */ (
        document.getElementById('edit-source-book-author')
      ).value = d.author || '';
      // @ts-ignore
      /** @type {HTMLInputElement} */ (
        document.getElementById('edit-source-book-isbn')
      ).value = d.isbn || '';

      // @ts-ignore
      /** @type {HTMLTextAreaElement} */ (
        document.getElementById('edit-source-book-websites')
      ).value =
        d.websites && Array.isArray(d.websites) ? d.websites.join('\n') : '';
      // @ts-ignore
      /** @type {HTMLTextAreaElement} */ (
        document.getElementById('edit-source-book-pdfs')
      ).value = d.pdfs && Array.isArray(d.pdfs) ? d.pdfs.join('\n') : '';
      break;

    case 'Website':
      // @ts-ignore
      /** @type {HTMLTextAreaElement} */ (
        document.getElementById('edit-source-website-websites')
      ).value =
        d.websites && Array.isArray(d.websites) ? d.websites.join('\n') : '';
      // @ts-ignore
      /** @type {HTMLTextAreaElement} */ (
        document.getElementById('edit-source-website-pdfs')
      ).value = d.pdfs && Array.isArray(d.pdfs) ? d.pdfs.join('\n') : '';
      break;
  }
}
...
export function initializeJournalSettingsHandlers() {
  const addSourceForm = /** @type {HTMLFormElement} */ (
  ...
  const editSourceForm = /** @type {HTMLFormElement} */ (
    document.getElementById('edit-source-form')
  );
  const newSourceTypeSelect = /** @type {HTMLSelectElement} */ (
    document.getElementById('new-source-type')
  );
  const editSourceTypeSelect = /** @type {HTMLSelectElement} */ (
    document.getElementById('edit-source-type')
  );
  ...
  if (addSourceForm) {
    addSourceForm.addEventListener('submit', async (e) => {
      ...
      const nameInput = /** @type {HTMLInputElement} */ (
        document.getElementById('new-source-name')
      );
      const typeInput = /** @type {HTMLSelectElement} */ (
        document.getElementById('new-source-type')
      );
      const urlInput = /** @type {HTMLInputElement} */ (
        document.getElementById('new-source-url')
      );
      const descriptionInput = /** @type {HTMLTextAreaElement} */ (
        document.getElementById('new-source-description')
      );
      const imagePathInput = /** @type {HTMLInputElement} */ (
        document.getElementById('new-source-image-path')
      );
      ...
      try {
        await addAdviceSource(newSourceData);
        // Re-fetch the *user-specific* list for the modal
        const userSources = await fetchAllAdviceSourcesForUser(
          state.selectedAccountHolderId
        );
    ...
    sourceList.addEventListener('click', async (e) => {
      ...
      if (editButton) {
        ...
        if (source && editSourceModal) {
          ...
          /** @type {HTMLInputElement} */ (
            document.getElementById('edit-source-name')
          ).value = source.name;
          /** @type {HTMLSelectElement} */ (
            document.getElementById('edit-source-type')
          ).value = source.type;
          /** @type {HTMLInputElement} */ (
            document.getElementById('edit-source-url')
          ).value = source.url || '';
          /** @type {HTMLTextAreaElement} */ (
            document.getElementById('edit-source-description')
          ).value = source.description || '';
          /** @type {HTMLInputElement} */ (
            document.getElementById('edit-source-image-path')
          ).value = source.image_path || '';
      ...
      if (deleteButton) {
        ...
        showConfirmationModal(
          'Remove Source?',
          'This will remove this source from your list, but will not delete it globally.',
          async () => {
            try {
    ...
    sourceList.addEventListener('change', async (e) => {
      ...
      if (target.matches('.edit-source-is-active')) {
        ...
        try {
          ...
          showToast(
            `Source ${isChecked ? 'activated' : 'deactivated'}.`,
            'success'
          );
  ...
  if (editSourceForm && editSourceModal) {
    editSourceForm.addEventListener('submit', async (e) => {
      ...
      const activeCheckbox = /** @type {HTMLInputElement} */ (
        document.getElementById(`active-toggle-${id}`)
      );
