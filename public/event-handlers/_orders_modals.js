...
export function initializeOrdersModalHandlers() {
  ...
  if (confirmFillForm) {
    confirmFillForm.addEventListener('submit', async (e) => {
      ...
      const executionDate = /** @type {HTMLInputElement} */ (
        document.getElementById('fill-execution-date')
      ).value;
      if (isNaN(executionPrice) || executionPrice <= 0 || !executionDate) {
        return showToast(
          'Please enter a valid positive Execution Price and Execution Date.',
          'error'
        );
      }
      submitButton.disabled = true;
      const pendingOrderId = /** @type {HTMLInputElement} */ (
        document.getElementById('fill-pending-order-id')
      ).value;
      const newTransaction = {
        account_holder_id: /** @type {HTMLInputElement} */ (
          document.getElementById('fill-account-holder-id')
        ).value,
        ticker: /** @type {HTMLInputElement} */ (
          document.getElementById('fill-ticker')
        ).value,
        exchange: /** @type {HTMLInputElement} */ (
      ...
      try {
        ...
        if (!updateRes.ok) {
          ...
          throw new Error(
            err.message || 'Failed to update pending order status.'
          );
        }
        ...
        if (!createRes.ok) {
          ...
          console.warn(
            `Transaction creation failed for filled order ${pendingOrderId}. Attempting rollback...`
          );
          ...
          throw new Error(
            err.message ||
              'Failed to create new transaction. Order status reverted.'
          );
        }
