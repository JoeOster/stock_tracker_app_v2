...
import {
  fetchAndStoreAdviceSources,
  fetchAllAdviceSourcesForUser,
} from './_journal_settings.js';
...
export async function populateSubscriptionPanel(holderId, holderName) {
  ...
  try {
    ...
    if (allSources.length === 0) {
      container.innerHTML =
        '<p>No global advice sources have been created by any user yet.</p>';
      return;
    }
...
export async function saveSubscriptions() {
  ...
  const sourceIds = Array.from(checkboxes).map(
    (cb) => /** @type {HTMLElement} */ (cb).dataset.sourceId
  );
