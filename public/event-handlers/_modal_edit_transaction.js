...
export function populateEditModal(tx, limitsOnly = false) {
  ...
  if (!tx || !editModal) {
    console.error(
      'populateEditModal: Transaction data or modal element not found.'
    );
  ...
  /** @type {HTMLInputElement} */ (document.getElementById('edit-id')).value =
    String(tx.id);
  /** @type {HTMLSelectElement} */ (
    document.getElementById('edit-account-holder')
  ).value = String(tx.account_holder_id);
  /** @type {HTMLInputElement} */ (document.getElementById('edit-date')).value =
    tx.transaction_date;
...
export function initializeEditTransactionModalHandler() {
  const editModal = document.getElementById('edit-modal');
  const editForm = /** @type {HTMLFormElement | null} */ (
  ...
  if (editForm && editModal) {
    ...
    editForm.addEventListener('submit', async (e) => {
      ...
      const limitUpExpInput = /** @type {HTMLInputElement} */ (
        document.getElementById('edit-limit-up-expiration')
      );
      const limitDownExpInput = /** @type {HTMLInputElement} */ (
        document.getElementById('edit-limit-down-expiration')
      );
      ...
      if (isNaN(quantity) || quantity <= 0 || isNaN(price) || price <= 0) {
        showToast(
          'Quantity and Price must be valid positive numbers.',
          'error'
        );
        return;
      }
      const limitUp = limitUpInput.value
        ? parseFloat(limitUpInput.value)
        : null;
      const limitDown = limitDownInput.value
        ? parseFloat(limitDownInput.value)
        : null;
      ...
      if (limitUp !== null && (isNaN(limitUp) || limitUp <= price)) {
        return showToast('Take Profit must be greater than Price.', 'error');
      }
      if (limitUp !== null && !limitUpExp) {
        return showToast('Take Profit needs an Expiration Date.', 'error');
      }
      if (
        limitDown !== null &&
        (isNaN(limitDown) || limitDown <= 0 || limitDown >= price)
      ) {
        return showToast(
          'Stop Loss must be positive and less than Price.',
          'error'
        );
      }
      if (limitDown !== null && !limitDownExp) {
        return showToast('Stop Loss needs an Expiration Date.', 'error');
      }

      const accountHolderId = /** @type {HTMLSelectElement} */ (
        document.getElementById('edit-account-holder')
      ).value;
      ...
      if (!accountHolderId || !ticker || !exchange) {
        showToast(
          'Account Holder, Ticker, and Exchange are required.',
          'error'
        );
        return;
      }
      ...
      const adviceSourceId = /** @type {HTMLSelectElement} */ (
        document.getElementById('edit-advice-source')
      ).value;
      ...
      const updatedTransaction = {
        ...
        ticker: ticker,
        exchange: exchange,
        transaction_type: /** @type {HTMLSelectElement} */ (
          document.getElementById('edit-type')
        ).value,
        quantity: quantity,
        price: price,
        transaction_date: /** @type {HTMLInputElement} */ (
        ...
        showConfirmationModal(
          ...
          async () => {
            try {
              ...
              if (managePositionModal?.classList.contains('visible')) {
                if (typeof openAndPopulateManageModal === 'function') {
                  await openAndPopulateManageModal(
                    ticker,
                    exchange,
                    accountHolderId
                  );
