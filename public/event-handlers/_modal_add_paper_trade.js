...
export function initializeAddPaperTradeModalHandler() {
    ...
    if (addJournalEntryForm && addJournalModal) {
        ...
        addJournalEntryForm.addEventListener('submit', async (e) => {
            ...
            if (isNaN(quantity) || quantity < 0) { return showToast('Quantity must be a valid positive number (or 0 for techniques).', 'error'); }
            if (isNaN(entryPrice) || entryPrice < 0) { return showToast('Entry Price must be a valid positive number (or 0 for techniques).', 'error'); }
            ...
            try {
                ...
                (/** @type {HTMLInputElement} */ (document.getElementById('journal-entry-date'))).value = getCurrentESTDateString(); // Reset date to today
