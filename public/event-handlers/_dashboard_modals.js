...
async function handleActionClick(e) {
  ...
  if (sellBtn) {
    const card = /** @type {HTMLElement} */ (
      sellBtn.closest('.dashboard-card[data-transaction-id]')
    );
    if (!card) return;
  ...
  if (selectiveSellBtn) {
    const card = /** @type {HTMLElement} */ (
      selectiveSellBtn.closest('.dashboard-card[data-ticker]')
    );
    if (!card) return;
    ...
    if (ticker && lotIds) {
      ...
      showToast(
        'Error: Could not find position data for selective sell.',
        'error'
      );
    }
    return; // Stop further processing
  ...
  if (manageBtn) {
    ...
    const card = /** @type {HTMLElement} */ (
      manageBtn.closest('.dashboard-card[data-ticker]')
    );
    if (!card) return;
...
export function initializeDashboardModalHandlers(dashboardContainer) {
  if (!dashboardContainer) {
    console.warn(
      'Dashboard container not found, cannot initialize modal handlers.'
    );
    return;
  }
