...
import { addSimpleWatchedTicker, deleteSimpleWatchedTicker } from '../api/watchlist-api.js';
// --- END ADDED ---
...
async function renderWatchlistPaperTrades(panelElement) {
    ...
    if (!openTableBody || !closedTableBody) {
        panelElement.innerHTML = '<h3>Paper Trades</h3><p style="color: var(--negative-color);">Error: Table body elements not found.</p>';
        return;
    }

    openTableBody.innerHTML = '<tr><td colspan="11">Loading open entries...</td></tr>';
    closedTableBody.innerHTML = '<tr><td colspan="10">Loading closed entries...</td></tr>';
    ...
    try {
        ...
        if (holderId) {
            ...
            results.forEach(result => {
                if (result.status === 'fulfilled') closedEntriesCombined.push(...result.value);
            });
            closedEntriesCombined.sort((a, b) => (b.exit_date || b.entry_date).localeCompare(a.exit_date || a.entry_date));
        } else {
             if (openTableBody) openTableBody.innerHTML = '<tr><td colspan="11">Please select an account holder to view journal entries.</td></tr>';
             if (closedTableBody) closedTableBody.innerHTML = '<tr><td colspan="10">Please select an account holder.</td></tr>';
        }

        updateState({ journalEntries: { openEntries: openEntries, closedEntries: closedEntriesCombined } });
        renderJournalPage({ openEntries, closedEntries: closedEntriesCombined }, true); // true = readOnly

    } catch (error) {
        console.error("Error loading paper trades for watchlist:", error);
        // @ts-ignore
        showToast(`Error loading paper trades: ${error.message}`, 'error');
        if (openTableBody) openTableBody.innerHTML = '<tr><td colspan="11">Error loading open entries.</td></tr>';
        if (closedTableBody) closedTableBody.innerHTML = '<tr><td colspan="10">Error loading closed entries.</td></tr>';
    }
}
...
export async function loadWatchlistPage() {
    const watchlistContainer = document.getElementById('watchlist-page-container');
    ...
    const activeSubTabId = activeSubTabButton instanceof HTMLElement ? activeSubTabButton.dataset.subTab : 'watchlist-real-panel';

    console.log(`[Watchlist Loader] Preparing to load content for sub-tab: ${activeSubTabId}`);

    const realPanel = /** @type {HTMLDivElement | null} */ (document.getElementById('watchlist-real-panel'));
    const paperPanel = /** @type {HTMLDivElement | null} */ (document.getElementById('watchlist-paper-panel'));
    const watchedPanel = /** @type {HTMLDivElement | null} */ (document.getElementById('watchlist-watched-panel'));
    ...
    try {
        ...
        console.error(`[Watchlist Loader] Error loading content for ${activeSubTabId}:`, error);
        // @ts-ignore
...
async function handlePaperTradeActions(e) {
    ...
    if (target.matches('#watchlist-add-paper-trade-btn')) {
        ...
        const form = /** @type {HTMLFormElement} */ (document.getElementById('add-journal-entry-form'));
        if (modal && form) {
            form.reset();
            (/** @type {HTMLInputElement} */(document.getElementById('journal-entry-date'))).value = getCurrentESTDateString();
            (/** @type {HTMLInputElement} */(document.getElementById('journal-form-entry-id'))).value = '';
            (/** @type {HTMLElement} */(document.getElementById('add-paper-trade-modal-title'))).textContent = 'Add New Journal Entry / Idea';
    ...
    if (target.matches('.journal-edit-btn')) {
        ...
        // @ts-ignore
        const openEntry = state.journalEntries?.openEntries.find(e => String(e.id) === journalId);
        // @ts-ignore
        const closedEntry = state.journalEntries?.closedEntries.find(e => String(e.id) === journalId);
        ...
        if (modal && form) {
            form.reset();
            (/** @type {HTMLElement} */(document.getElementById('add-paper-trade-modal-title'))).textContent = `Edit Entry: ${entry.ticker}`;
            (/** @type {HTMLButtonElement} */(document.getElementById('add-journal-entry-btn'))).textContent = 'Save Changes';
            (/** @type {HTMLInputElement} */(document.getElementById('journal-form-entry-id'))).value = entry.id;
            (/** @type {HTMLInputElement} */(document.getElementById('journal-entry-date'))).value = entry.entry_date;
            (/** @type {HTMLInputElement} */(document.getElementById('journal-ticker'))).value = entry.ticker;
            (/** @type {HTMLSelectElement} */(document.getElementById('journal-exchange'))).value = entry.exchange;
            (/** @type {HTMLSelectElement} */(document.getElementById('journal-direction'))).value = entry.direction;
            (/** @type {HTMLInputElement} */(document.getElementById('journal-quantity'))).value = String(entry.quantity);
            (/** @type {HTMLInputElement} */(document.getElementById('journal-entry-price'))).value = String(entry.entry_price);
            (/** @type {HTMLInputElement} */(document.getElementById('journal-target-price'))).value = entry.target_price || '';
            (/** @type {HTMLInputElement} */(document.getElementById('journal-target-price-2'))).value = entry.target_price_2 || '';
            (/** @type {HTMLInputElement} */(document.getElementById('journal-stop-loss-price'))).value = entry.stop_loss_price || '';
            (/** @type {HTMLInputElement} */(document.getElementById('journal-advice-details'))).value = entry.advice_source_details || '';
            (/** @type {HTMLInputElement} */(document.getElementById('journal-entry-reason'))).value = entry.entry_reason || '';
            (/** @type {HTMLTextAreaElement} */(document.getElementById('journal-notes'))).value = entry.notes || '';
            await populateAllAdviceSourceDropdowns();
            (/** @type {HTMLSelectElement} */(document.getElementById('journal-advice-source'))).value = entry.advice_source_id || '';
            modal.classList.add('visible');
        }
    }
}
...
async function handleWatchedTickerActions(e) {
    ...
    if (target.matches('#add-watched-ticker-form') || target.closest('#add-watched-ticker-form')) {
        e.preventDefault(); // Prevent form submission
        const input = /** @type {HTMLInputElement} */ (document.getElementById('add-watched-ticker-input'));
        ...
        try {
            const result = await addSimpleWatchedTicker(ticker, state.selectedAccountHolderId);
            showToast(result.message, 'success');
            input.value = ''; // Clear input
            // Refresh this sub-tab
            await renderWatchedTickers(/** @type {HTMLDivElement} */ (document.getElementById('watchlist-watched-panel')));
        } catch (error) {
            // @ts-ignore
    ...
    if (target.matches('.delete-watched-ticker-btn')) {
        ...
        showConfirmationModal(`Stop watching ${ticker}?`, 'Are you sure you want to remove this ticker from your watched list?', async () => {
            try {
...
export function initializeWatchlistHandlers() {
    const watchlistContainer = document.getElementById('watchlist-page-container');
    const watchlistSubTabsContainer = watchlistContainer?.querySelector('.watchlist-sub-tabs');
