...
export function populateSelectiveSellModal(ticker, lots) {
  ...
  ).value = ''; // Clear previous total
  /** @type {HTMLInputElement} */ (
    document.getElementById('selective-sell-total-quantity')
  ).max = String(totalQuantity); // Set max based on available
  /** @type {HTMLInputElement} */ (
  ...
  ).textContent = '0'; // Reset selected total
  /** @type {HTMLElement} */ (
    document.getElementById('selective-sell-validation-message')
  ).style.display = 'none'; // Hide validation
  ...
  const validateQuantities = () => {
    ...
    if (totalsMatch) {
      ...
      if (targetTotal > 0 && selectedTotal > 0) {
        // Only show message if user has started entering values
    ...
    selectiveSellForm.addEventListener('submit', async (e) => {
      ...
      const lotsPayload = lotInputs
        .map((input) => ({
          ...
          quantity_to_sell:
            parseFloat(/** @type {HTMLInputElement} */ (input).value) || 0, // Default to 0 if empty/invalid
        }))
