...
export async function loadDailyReportPage(viewValue) {
    ...
    try {
        ...
        const percentage = (previousValue !== 0) ? (dailyChange / previousValue * 100) : 0;
...
export function initializeDailyReportHandlers() {
    try {
        ...
        if (dailyReportContainer && sellFromPositionModal && editModal && adviceModal) {
            dailyReportContainer.addEventListener('click', (e) => {
                ...
                if (row && !target.closest('button')) {
                    ...
                    if (!lotData) {
                        console.error("[DailyReport Event] Lot data not found in activityMap for key:", lotKey); // Keep error log
                        return;
                    }
                    ...
                    const adviceCurrentPrice = document.getElementById('advice-current-price');
                    const adviceSuggestedProfit = document.getElementById('advice-suggested-profit');
                    const adviceSuggestedLoss = document.getElementById('advice-suggested-loss');
                    const adviceProfitPercent = document.getElementById('advice-profit-percent');
                    const adviceLossPercent = document.getElementById('advice-loss-percent');
                    const adviceCurrentLimitUp = document.getElementById('advice-current-limit-up');
                    const adviceCurrentLimitDown = document.getElementById('advice-current-limit-down');

                    if (!adviceTitle || !adviceCostBasis || !adviceCurrentPrice || !adviceSuggestedProfit || !adviceSuggestedLoss || !adviceProfitPercent || !adviceLossPercent || !adviceCurrentLimitUp || !adviceCurrentLimitDown) {
                       console.error("[DailyReport Event] One or more elements missing inside advice modal."); // Keep error log
                    ...
                    adviceCurrentPrice.textContent = (priceData && typeof priceData.price === 'number') ? formatAccounting(priceData.price) : 'N/A';
                ...
                if (sellBtn) {
                    ...
                    const sellDateInput = /** @type {HTMLInputElement} */(document.getElementById('sell-date'));
                     ...
                     if (!sellParentBuyIdInput || !sellAccountHolderIdInput || !sellTickerDisplay || !sellExchangeDisplay || !sellQuantityInput || !sellDateInput) {
                       console.error("[DailyReport Event] One or more elements missing inside sell modal."); // Keep error log
                ...
                if (setLimitBtn || editBuyBtn) {
                    ...
                    if (lotData) {
                        ...
                        if (!editIdInput || !editAccountHolderSelect || !editDateInput || !editTickerInput || !editExchangeSelect || !editTypeSelect || !editQuantityInput || !editPriceInput || !editLimitPriceUpInput || !editLimitUpExpirationInput || !editLimitPriceDownInput || !editLimitDownExpirationInput || !coreFields || !limitFields || !modalTitle) {
                             console.error("[DailyReport Event] One or more elements missing inside edit modal."); // Keep error log
                        ...
                        editModal.classList.add('visible');
                         ...
                         console.error("[DailyReport Event] Lot data not found in activityMap for ID:", id); // Keep error log
                    }
                }
            });
        } else {
            console.error("[DailyReport Init] Could not find required elements to attach listener."); // Keep error log
        }
    } catch (error) {
        console.error("[DailyReport Init] CRITICAL ERROR during initialization:", error); // Keep error log
    }
}
