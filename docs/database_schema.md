...
This document outlines the final, consolidated schema for the Portfolio Tracker application database, based on the initial setup and all applied migrations. Planned changes for future phases are noted.
...
| `advice_source_id`  | INTEGER | FOREIGN KEY (advice_sources.id) SET NULL | **(DEPRECATED?)** Links to a strategy or advice source. Consider removing? |
...
| `contact_email`    | TEXT    |                                           |                                                                                 |
| `contact_phone`    | TEXT    |                                           |                                                                                 |
| `contact_app`      | TEXT    |                                           | **(TO BE REPLACED)** e.g., Discord handle, Slack ID.                           |
| `contact_app_type` | TEXT    |                                           | **(PLANNED)** Dropdown value (e.g., 'WhatsApp', 'Signal', 'email').           |
| `contact_app_handle`| TEXT   |                                           | **(PLANNED)** Corresponding handle/ID/address.                                |
...
Stores journal entries (paper trades / tracked advice). **(Purpose may shift towards paper trading only)**
...
Stores links to external documents (charts, notes) related to journal entries. **(May need to link to advice_sources too)**

| Column           | Type    | Constraints                               | Description                                     |
| :--------------- | :------ | :---------------------------------------- | :---------------------------------------------- |
| `id`             | INTEGER | PRIMARY KEY AUTOINCREMENT                 |                                                 |
| `journal_entry_id`| INTEGER| FOREIGN KEY (journal_entries.id) CASCADE| **(EXISTING)** Link to a journal entry.         |
| `advice_source_id`| INTEGER| FOREIGN KEY (advice_sources.id) CASCADE | **(PLANNED)** Link directly to an advice source. |
| `title`          | TEXT    |                                           |                                                 |
...
| `account_holder_id` | INTEGER | NOT NULL, FOREIGN KEY                   |                                                |
| `ticker`            | TEXT    | NOT NULL                                |                                                |
| `advice_source_id`  | INTEGER | FOREIGN KEY (advice_sources.id) SET NULL | **(PLANNED)** Link to source if added via Source tab.|
| `created_at`        | TEXT    | NOT NULL DEFAULT CURRENT_TIMESTAMP      |                                                |
|                     |         | UNIQUE (account_holder_id, ticker)      |                                                |

---

### `source_notes` **(PLANNED)**

**(New Table)** Stores notes specifically related to an advice source, separate from journal entry notes.
